/**
 * @file LevelData.js
 * @description Data structure and parser for Yag2Dpg levels.
...
class LevelData {
    ...
    static async fromZip(file) {
        ...
        // 3. Parse map.js (Dynamically import it via Blob URL to execute its ES6 exports safely)
            let mapFile = zip.file(rootFolder + 'map.js');
            if (mapFile) {
                let mapText = await mapFile.async('string');
                
                /**
                 * ENGINE RESILIENCE HACK
                 * Dynamically injects a failsafe ES6 default export mechanism at runtime 
                 * to support legacy level configurations lacking explicit export declarations.
                 * Uses typeof checks to prevent ReferenceErrors if a map omits certain variables.
                 */
                ...
                try {
                    ...
                    /**
                     * Safely resolve the exported data whether it comes from the dynamic injection
                     * (default export) or canonical named exports from the source file.
                     * @type {Object}
                     */
                    ...
                    URL.revokeObjectURL(url); // Trigger immediate garbage collection
                }
            } else {
        ...
        if (imgPath) {
            ...
            instance.bgImage = await new Promise((resolve) => {
                loadImage(imgUrl, (loadedImg) => resolve(loadedImg));
            });
        }

        // 5. Load Background Music (Optional)
        ...
        if (musicPath) {
            ...
            let musicUrl = URL.createObjectURL(musicBlob);
            instance.bgMusic = await new Promise((resolve) => {
                loadSound(musicUrl, (loadedSnd) => resolve(loadedSnd));
            });
        }
    ...
    static #parseLicense(text) {
        let parsed = {};
        // Split text using the "-- Section Name --" format as delimiter
